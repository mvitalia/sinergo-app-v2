...
dim connessione
connessione = "DRIVER={Microsoft Access Driver (*.mdb)};DBQ=" & server.MapPath("App_data/Promo.mdb") 
...
if not rsCorsi.EOF then
	while not rsCorsi.EOF
		...
		corsi(null)("data") = rsCorsi.Fields("data_inserimento")
		corsi(null)("pdf") = rsCorsi.Fields("pdf")
		corsi(null)("stato") = rsCorsi.Fields("stato")
		corsi(null)("link") = rsCorsi.Fields("link")
		rsCorsi.MoveNext()
	wend
end if
